...
for (const chunk of CHUNKS) {
  ...
  await page.evaluate((off) => {
    ...
    if (off.includes('trees')) window.civ.setStreetTrees(false)
    if (off.includes('reflections')) window.civ.setReflections(false)
    window.civ.freezeClock(120)
    window.civ.rig.settle()
  }, [...OFF])
  await page.waitForTimeout(1400)
  await page.screenshot({ path: `${OUT}/${chunk}.png` })
  const tc = await page.evaluate(() => ({ ...window.civ.trafficCount(), trees: window.civ.streetTreeCount(), refl: window.civ.reflectionCount() }))
  console.log(`${chunk}: ${n} owned, traffic ${tc.total} (${tc.boats} boats), trees ${tc.trees}, refl ${tc.refl} -> ${OUT}/${chunk}.png`)
  await page.close()
}
await browser.close()
