...
for (const chunk of CHUNKS) {
  ...
  const tc = await page.evaluate(() => ({ ...window.civ.trafficCount(), trees: window.civ.streetTreeCount() }))
  console.log(`${chunk}: ${n} owned, traffic ${tc.total} (${tc.boats} boats), trees ${tc.trees} -> ${OUT}/${chunk}.png`)
  await page.close()
}
await browser.close()
