...
test(
  ...
  { skip: URL ? false : 'set CIV_TEST_DATABASE_URL to run the durable resume test' },
  async () => {
    await scrub()
    ...
    assert.equal(new Set([...firstIds, ...secondIds]).size, 80, 'and no id was issued twice')
  },
)
...
test(
  ...
  async () => {
    ...
    const first = new DurableStore({ url: URL, flushMs: 50 })
    assert.equal((await first.resumePoint(CHUNK)).season, 1, 'a chunk that has never run is at 1')
...
test(
  ...
  async () => {
    ...
    assert.equal(kept.events, 0, 'a keep window wider than the world has lived deletes nothing')

    const pruned = await store.pruneSeasons(CHUNK, 2)
    assert.equal(pruned.events, 5, 'keep=2 on season 3 drops season 1 and nothing else')
...
test(
  ...
  async () => {
    ...
    const back = await new DurableStore({ url: URL, flushMs: 50 }).resumePoint(CHUNK)
